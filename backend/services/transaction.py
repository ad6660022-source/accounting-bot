...
Все операции выполняются атомарно в рамках одной транзакции БД.
"""
...
async def process_operation(
    ...
    Возвращает созданную запись Transaction.

    Логика по типам:
      zakup:       user.cash -= amount, ip.cash += amount
      storonnie:   user.cash -= amount
      prihod_*:    user.cash += amount
      snyat_rs:    ip.bank -= amount, user.cash += amount
      vnesti_rs:   user.cash -= amount, ip.bank += amount
      odolzhit:    creditor.cash -= amount, debtor.cash += amount, долг создаётся в debt.py
    """
...
async def repay_debt_operation(
    ...
    async with session.begin():
        ...
        # Переводим деньги должника кредитору
